...
module.exports = (app) => {
	...
	//http://localhost:5595/account/api/public/account?accountId=Galaxy10&accountId=Galaxy11&accountId=Galaxy12&accountId=Galaxy13&accountId=Galaxy14&accountId=Galaxy15
	...
	app.get('/account/api/public/account/', (req, res) => {
		try {
			...
			if (`${req.query.accountId}`.startsWith("Galaxy")) { displayName = "Galaxy" }
			...
			req.query.accountId.forEach(accId => {
				var dn = accId
				if (`${accId}`.startsWith("Galaxy")) { dn = "Galaxy" }
				response.push({
					id: accId,
					displayName: dn,
					externalAuths: {}
