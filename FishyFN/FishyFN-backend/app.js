...
(function () {
	...
	try {
		global.BotConfig = JSON.parse(fs.readFileSync('./config/TropicalBOT/config.json', 'utf8', function (err, data) {
	...
	if (!BotConfig) {
		fs.mkdirSync(`./config/TropicalBOT/`, { recursive: true });
		fs.writeFile(`./config/TropicalBOT/config.json`, JSON.stringify({
			skin: 'CID_811_Athena_Commando_F_CandySummer',
			emote: null
		}), function (err, data) {
			if (err) global.BotConfig = false;
		})
	...
	app.listen(port, () => {
		console.log(`FishyFN is ready to arise on port 5955! Welcome To The Center Of Fish!`);
	});
